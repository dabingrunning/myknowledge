...
/**
 * Created by myb on 2017/4/18.
 */

public class WaveAnimat extends View {
    ...
    // 第三条水波移动速度
    private static final int TRANSLATE_X_SPEED_THREE = 1;
    ...
    public WaveAnimat(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < mTotalWidth; i++) {
            mYPositions[i] = (float) (max * Math.sin(mCycleFactorW * i) + OFFSET_Y);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mTotalWidth; i++) {
            ...
            canvas.drawLine(i, mTotalHeight - mResetTwoYPositions[i], i,
                    ...
                    mWavePaint);
            // 绘制第三条水波纹
            canvas.drawLine(i, mTotalHeight - mResetTHREEYPositions[i], i,
                    mTotalHeight,
                    mWavePaint);
        }
